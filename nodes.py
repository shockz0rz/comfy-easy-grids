    ...
    def INPUT_TYPES(s):
        return { 
            "required": {
                ...
                "float3": ("FLOAT", {"default": 1.0}),
                "float4": ("FLOAT", {"default": 1.0}),
                "float5": ("FLOAT", {"default": 1.0}),
                "float6": ("FLOAT", {"default": 1.0}),
            },
             }
...
class SimpleLooper:
    @classmethod
    def INPUT_TYPES(s):
        return { "required": { "loop_count": ("INT", {"default":1, "min": 1, "max": 10, "step": 1}),}}
...
class FloatToText:
    ...
    def convert_to_str(self, float_input, decimal_places):
...
class SaveImageGrid:
    ...
    def accumulate_images(self, images, x_size, y_size, filename_prefix="ComfyUI", prompt=None, extra_pnginfo=None):
        filename_prefix += self.prefix_append
        if x_size != self.curr_x_size or y_size != self.curr_y_size:
    ...
    def assemble_grid(self):
        ...
        grid_canvas = Image.new("RGBA", (space_width * self.curr_x_size, space_height * self.curr_y_size), color="#ffffff00")
        for y_idx in range( self.curr_y_size ):
            for x_idx in range( self.curr_x_size ):
                ...
                grid_canvas.paste(pil_image, (x_idx * space_width, y_idx * space_height ))
        return grid_canvas
